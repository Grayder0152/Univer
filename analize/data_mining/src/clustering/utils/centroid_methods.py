...
class CentroidMethod(ABC):
    ...
    @abstractmethod
    def get_centroids(self, data: pl.DataFrame) -> pl.DataFrame:
        pass
...
class KMeanPp(CentroidMethod):
    ...
    def get_centroids(self, dataframe: pl.DataFrame) -> np.array:
...
class Random(CentroidMethod):
    ...
    def get_centroids(self, dataframe: pl.DataFrame) -> np.array:
        random_indices = np.random.choice(dataframe, size=self.k, replace=False)
        init_centroids = dataframe[random_indices].to_numpy()
        return init_centroids


class _CentroidManager(type):
...
class CentroidManager(metaclass=_CentroidManager):
    pass
